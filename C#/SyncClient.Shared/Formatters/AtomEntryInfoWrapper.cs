            ...
            else
            {
                ...
                if (string.IsNullOrEmpty(this.Id) && this.TempId == null)
                    ...
                    throw new InvalidOperationException(
                        "A atom:id or a sync:tempId element must be present. Entity in error: " +
                        entry.ToString(SaveOptions.None));
                ...
                foreach (XElement linkElement in entry.Elements(FormatterConstants.AtomXmlNamespace + FormatterConstants.AtomPubLinkElementName))
                {
                    ...
                    {
                        if (attr.Value.Equals(FormatterConstants.AtomPubEditLinkAttributeName, StringComparison.CurrentCultureIgnoreCase))
                        {
                            ...
                            if (hrefAttr == null)
                            {
                                ...
                                throw new InvalidOperationException(
                                    string.Format("No href attribute found in the edit link for atom with  {0}: '{1}'",
                                        (this.Id == null) ? "TempId" : "Id", this.Id ?? this.TempId));
                            }
